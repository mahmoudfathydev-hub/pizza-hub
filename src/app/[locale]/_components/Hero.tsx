import Link from "@/src/components/link"
import { buttonVariants } from "@/src/components/ui/button"
import { Routes } from "@/src/constants/enums"
import { ArrowRightCircle } from "lucide-react"
import Image from "next/image"

const Hero = () => {
    return (
        <section className="section-gap">
            <div className="container grid grid-cols-1 md:grid-cols-2">
                <div className="text md:py-12 ">
                    <h1 className="text-4xl font-semibold">Slice Into Happiness</h1>
                    <p className="text-accent my-4">craving Pizza? We&apos;ve got you craverd with fresh ingredients, endless flavors, and the fastest delivery service in town. Your perfect pizza is just a click away!</p>
                    <div className="btns flex items-center gap-4">
                        <Link 
                        href={`/${Routes.MENU}`}
                        className={`${buttonVariants({ size: "lg" })} 
                        space-x-2 px-4! rounded-full! uppercase`}>
                            Order Now 
                            <ArrowRightCircle className={`w-5! h-5!`} />
                        </Link>
                        <Link 
                        href={`/${Routes.ABOUT}`}
                        className="flex gap-2 items-center text-black hover:text-primary duration-300 transition-all font-semibold">
                            Learn More 
                            <ArrowRightCircle className={`w-5! h-5!`} />
                        </Link>
                    </div>
                </div>
                <div className="img relative hidden md:block">
                    <Image src="/pizza.png" alt="Pizza" fill loading="eager" priority className="object-contain"/>
                </div>
            </div>
        </section>
    )
}

export default Hero